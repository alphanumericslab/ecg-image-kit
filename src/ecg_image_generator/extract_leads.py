...
from  ecg_image_generator.helper_functions import get_adc_gains,get_frequency,get_leads,load_recording,load_header,find_files, truncate_signal, create_signal_dictionary, standardize_leads, write_wfdb_file
...
def get_paper_ecg(input_file,header_file,output_directory, seed, add_dc_pulse,add_bw,show_grid, add_print, configs, mask_unplotted_samples = False, start_index = -1, store_configs=False, store_text_bbox=True,key='val',resolution=100,units='inches',papersize='',add_lead_names=True,pad_inches=1,template_file=os.path.join('TemplateFiles','TextFile1.txt'),font_type=os.path.join('Fonts','Times_New_Roman.ttf'),standard_colours=5,full_mode=['II'],bbox = False,columns=-1, store_plotted_pixels=False):
    ...
    if(len(full_leads)==2):
        ...
        gen_m = 12
        for i, leadName in enumerate(full_mode):
            if leadName not in full_leads:
                full_mode[i] = full_leads[i%len(full_leads)]
        else:
            full_mode = full_mode
        if(columns==-1):
            columns = 4
    ...
    if start_index != -1:
        ...
        for key in record_dict:
            if(len(record_dict[key][start:])<int(rate*abs_lead_step)):
                ...
                if(full_mode!='None' and key in full_mode):
                    if 'full'+key not in segmented_ecg_data.keys():
                        segmented_ecg_data['full'+key] = nanArray.tolist()
                    else:
                        segmented_ecg_data['full'+key] = segmented_ecg_data['full'+key] + nanArray.tolist()
                ...
                if(full_mode!='None' and key in full_mode):
                    if(len(record_dict[key][start:])>int(rate*10)):
                        frame['full'+key] = record_dict[key][start:(start+int(rate)*10)]
                        frame['full'+key] = frame['full'+key]
                        if 'full'+key not in segmented_ecg_data.keys():
                            segmented_ecg_data['full'+key] = frame['full'+key].tolist()
                        else:
                            segmented_ecg_data['full'+key] = segmented_ecg_data['full'+key] + frame['full'+key].tolist()
                    else:
                        frame['full'+key] = record_dict[key][start:]
                        frame['full'+key] = frame['full'+key]
                        if 'full'+key not in segmented_ecg_data.keys():
                            segmented_ecg_data['full'+key] = frame['full'+key].tolist()
                        else:
