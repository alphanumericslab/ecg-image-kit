...
def ecg_plot(
    ...
    for i in np.arange(len(lead_index)):
        ...
        if (bbox):
            ...
            lead_bbox.append({'x1':int(x1), 'y1': int(y1), 'x2': int(x2), 'y2': int(y2), 'class': 0, 'lead': leadName})
    ...
    if(full_mode!='None'):
        ...
        if (bbox):
            ...
            lead_bbox.append({'x1':int(x1), 'y1': int(y1), 'x2': int(x2), 'y2': int(y2), 'class': 1, 'lead': full_mode})
